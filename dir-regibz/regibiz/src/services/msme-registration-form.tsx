  ...
  return (
    <div className="bg-gradient-to-r from-emerald-900/30 to-emerald-800/10 border border-emerald-500/20 rounded-xl p-4 md:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-lg mb-8 relative overflow-hidden backdrop-blur-sm">
      ...
      <div className="text-left sm:text-right z-10">
        ...
        <p className="text-slate-100 font-mono font-bold text-lg md:text-xl tracking-wider">MSME-2026-8X92B</p> {/* 🔥 BIGGER & BOLDER */}
      </div>
    </div>
  );
};
    ...
    <div className="space-y-6 hidden lg:block">
      ...
      <div className="bg-slate-900/70 backdrop-blur-xl border border-slate-700/50 rounded-xl p-5 shadow-xl transition-all duration-300">
        <h3 className="text-white text-sm font-semibold mb-4 flex items-center">
          <span className="bg-sky-500/20 p-1.5 rounded mr-2">
        ...
        <div className="relative border-l-2 border-slate-700/60 ml-2 space-y-5 my-2">
          {[1, 2, 3].map((step) => (
            <div key={step} className="ml-5 relative">
              <span
                className={`absolute -left-[27px] w-3.5 h-3.5 rounded-full border-2 border-slate-800 transition-all duration-300 ${
      ...
      {/* Required Documents */}
      <div
        className={`bg-slate-900/70 backdrop-blur-xl border border-slate-700/50 rounded-xl p-5 shadow-xl transition-opacity duration-300 ${
          currentStep === 3 ? 'opacity-100 ring-1 ring-emerald-500/30' : 'opacity-90'
        }`}
      >
        ...
        <ul className="space-y-3">
          {[
            ...
            return (
              <li key={idx} className="flex items-center justify-between py-2 px-3 bg-slate-800/30 rounded-lg hover:bg-slate-800/50 transition-colors group">
                <div className="flex items-center">
                  <div className={`mr-3 w-5 h-5 rounded-full flex items-center justify-center transition-all ${
                    isUploaded 
                  ...
                  <span className={`font-medium ${isUploaded ? 'text-slate-100' : 'text-slate-300'}`}>
                    {item.label}
                  </span>
                </div>
                {isUploaded && (
      ...
      <div className="bg-slate-900/70 backdrop-blur-xl border border-slate-700/50 rounded-xl p-5 shadow-xl">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between p-3 bg-slate-800/40 rounded-lg border border-slate-700/50">
            <span className="text-slate-300 font-medium">Toll Free</span>
            <span className="font-mono font-bold text-emerald-400 text-base tracking-tight">1800-103-4786</span>
          </div>
          <div className="flex items-center justify-between p-3 bg-slate-800/40 rounded-lg border border-slate-700/50">
            <span className="text-slate-300 font-medium">Email</span>
            <span className="font-mono font-bold text-sky-400 text-base">he lp@msme.gov</span>
          </div>
        </div>
      </div>
      ...
      <div className="pt-2 flex justify-center">
        <button
          onClick={onPreview}
          className="w-full py-4 rounded-xl bg-gradient-to-r from-slate-800 to-slate-700 border border-slate-600/50 text-emerald-400 font-bold text-base tracking-wide shadow-lg hover:bg-slate-600 hover:text-white hover:border-emerald-500/50 transition-all duration-300 flex items-center justify-center gap-2 group"
        >
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-900 rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto border border-slate-700 shadow-2xl">
        ...
        <div className="p-6 space-y-6">
          <div>
            <h4 className="font-bold text-emerald-400 text-lg mb-2">Business Profile</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div><span className="text-slate-500 font-medium">Enterprise Name:</span> <span className="text-white font-medium">{formData.enterpriseName || '—'}</span></div>
              <div><span className="text-slate-500 font-medium">PAN:</span> <span className="text-white font-mono font-bold">{formData.pan || '—'}</span></div>
              <div><span className="text-slate-500 font-medium">Organisation Type:</span> <span className="text-white font-medium">
                {orgTypeOptions.find(o => o.value === formData.orgType)?.label || '—'}
              </span></div>
              <div><span className="text-slate-500 font-medium">Major Activity:</span> <span className="text-white font-medium">
                {majorActivityOptions.find(o => o.value === formData.majorActivity)?.label || '—'}
              </span></div>
              <div><span className="text-slate-500 font-medium">Social Category:</span> <span className="text-white font-medium">
                {socialCategoryOptions.find(o => o.value === formData.socialCategory)?.label || '—'}
              </span></div>
            </div>
          </div>
          <div>
            <h4 className="font-bold text-emerald-400 text-lg mb-2">Contact & Address</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div><span className="text-slate-500 font-medium">Email:</span> <span className="text-white font-mono">{formData.email || '—'}</span></div>
              <div><span className="text-slate-500 font-medium">Mobile:</span> <span className="text-white font-mono">{formData.mobile || '—'}</span></div>
              <div><span className="text-slate-500 font-medium">Address:</span> <span className="text-white font-medium">{formData.addressLine1 || '—'}</span></div>
              <div><span className="text-slate-500 font-medium">City / State / PIN:</span> <span className="text-white font-mono">
                {formData.city} / {stateOptions.find(s => s.value === formData.state)?.label || '—'} / {formData.pincode || '—'}
              </span></div>
            </div>
          </div>
          <div>
            <h4 className="font-bold text-emerald-400 text-lg mb-2">Documents Uploaded</h4>
          ...
          <div className="pt-4 border-t border-slate-800">
            <p className="text-sm text-slate-500">
              This is a preview only. No data is submitted until you click <span className="text-emerald-400 font-bold">Proceed to Payment</span>.
            </p>
          </div>
        </div>
        <div className="p-6 bg-slate-800/50 flex justify-end gap-3">
          ...
          <button
            ...
            className={`px-6 py-2 rounded-lg font-bold text-base transition-all ${
              isSubmitting
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-2xl shadow-2xl max-w-md w-full text-center border border-slate-700/50">
          ...
          <p className="text-slate-300 mb-8">
            ...
            <span className="text-sky-400 font-mono font-bold text-lg">MSME-2026-8X92B</span>.
          </p>
          <button
                    ...
                    <div>
                      ...
                      <div className="p-4 bg-slate-800/40 rounded-xl border border-slate-700/50 flex flex-col sm:flex-row items-center justify-between">
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="bg-slate-900 px-4 py-2 rounded-lg border border-slate-700 font-mono text-xl font-bold text-sky-400 tracking-wider">
