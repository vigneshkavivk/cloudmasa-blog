...
import GstComplianceGuide from "./pages/GstComplianceGuide";
...
const ProfileCompletionModal: React.FC<{
  user: UserProfile;
  onClose: () => void;
  onSave: (data: Partial<UserProfile>) => void; // ✅ Added "data"
  forced?: boolean;
}> = ({ user, onClose, onSave, forced = false }) => {
  ...
  // OTP States (only for CUSTOMER role)
  const [otpStep, setOtpStep] = useState<'request' | 'verify' | 'verified'>('request');
  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (otpStep === 'verified' && e.target.value !== user.phoneNumber) {
      setOtpStep('request');
    }
  };
  ...
  const requestOtp = () => {
    ...
    setMockOtp(code);
    setOtpStep('verify');
    alert(`RegiBIZ Security: Your verification code is ${code}.`);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isCustomer && otpStep !== 'verified') {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
      <div className="bg-navy-900 rounded-2xl border border-white/10 w-full max-w-md p-6 shadow-2xl relative">
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-white border border-white/10">
            <User size={20} />
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Full Name */}
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5 ml-1">Full Legal Name</label>
            <input
              required
              type="text"
          ...
          {isCustomer && (
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5 ml-1">Personal Mobile Number</label>
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full bg-navy-800 border border-white/10 rounded-lg pl-9 pr-4 py-2.5 text-white focus:border-orange-500 focus:outline-none text-sm transition-colors"
                ...
                {otpStep === 'request' && (
                  <button
                    type="button"
                    onClick={requestOtp}
                    className="px-3 py-2 bg-orange-500/10 text-orange-400 hover:bg-orange-500 hover:text-white rounded-lg text-xs font-bold border border-orange-500/20 transition-all whitespace-nowrap"
                  >
                    Verify
                  </button>
                )}
                {otpStep === 'verify' && (
                  <button
                    type="button"
                    disabled
                    className="px-3 py-2 bg-gray-800 text-gray-500 rounded-lg text-xs font-bold border border-gray-700 cursor-not-allowed whitespace-nowrap"
                  >
                    Sent
                  </button>
                )}
                {otpStep === 'verified' && (
                  <div className="px-3 py-2 bg-emerald-500/10 text-emerald-500 rounded-lg text-xs font-bold border border-emerald-500/20 flex items-center gap-1">
              ...
              {otpStep === 'verify' && (
                <div className="mt-3 animate-fade-in p-3 bg-white/5 rounded-lg border border-white/5">
                  <label className="block text-[10px] text-gray-400 mb-1">Enter Code</label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={otpInput}
                      onChange={(e) => setOtpInput(e.target.value)}
                      className="flex-1 bg-navy-900 border border-white/10 rounded px-3 py-1.5 text-white text-sm tracking-widest text-center focus:border-orange-500 outline-none"
                      placeholder="XXXX"
                      autoFocus
                    />
                    <button
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  <Heart size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
                  <select
                    value={formData.gender}
                    onChange={(e) => setFormData({ ...formData, gender: e.target.value })}
                    className="w-full bg-navy-800 border border-white/10 rounded-lg pl-9 pr-4 py-2.5 text-white focus:border-orange-500 focus:outline-none text-sm appearance-none cursor-pointer"
                  >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-navy-800 border border-white/10 rounded-lg pl-9 pr-4 py-2.5 text-white focus:border-orange-500 focus:outline-none text-sm"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="pt-6 flex justify-between items-center gap-3">
            ...
            <div className={`flex gap-3 ${forced ? 'w-full justify-end' : ''}`}>
              <button
                type="submit"
                disabled={saving || (isCustomer && otpStep !== 'verified')}
                className="px-6 py-2.5 bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white rounded-lg text-sm font-bold shadow-lg shadow-orange-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {saving ? 'Saving...' : 'Complete Profile'} <ChevronDown className="rotate-[-90deg]" size={14} />
              </button>
            </div>
          </div>
        </form>
...
// ─── Header Component (with Back Button & Notifications) ──────────────
const Header: React.FC<{
  user: UserProfile;
  toggleSidebar: () => void;
  isCollapsed: boolean;
  openProfile: () => void;
}> = ({ user, toggleSidebar, isCollapsed, openProfile }) => {
  ...
  const getPageTitle = () => {
    ...
    if (path === '/') return user.role === UserRole.CUSTOMER ? 'Dashboard Overview' : 'Staff Dashboard';
    if (path === '/services') return 'Service Hub';
    if (path === '/documents') return 'My Documents';
    if (path === '/calendar') return 'Compliance Calendar';
    if (path === '/admin') return 'User Management';
    if (path === '/consult') return 'Consultation';
    if (path.includes('/fssai-license')) return 'FSSAI License';
    if (path.includes('/gst-registration')) return 'GST Registration';
    if (path.includes('/msme-registration')) return 'MSME Registration';
    return 'RegiBIZ';
  };
  ...
  const handleGoBack = () => {
    if (location.pathname.includes('/fssai-license/form')) navigate('/services/fssai-license');
    else if (location.pathname.includes('/gst-registration/form')) navigate('/services/gst-registration');
    else if (location.pathname.includes('/msme-registration/form')) navigate('/services/msme-registration');
    else if (location.pathname.includes('/trademark-registration/form')) navigate('/services/trademark-registration');
    else if (location.pathname.includes('/pan-registration/form')) navigate('/services/pan-registration');
    else navigate(-1);
  };
  ...
  return (
    <header className="sticky top-0 z-30 h-16 glass-panel border-b border-white/5 px-4 md:px-8 flex items-center justify-between bg-[#0a192f]/80 backdrop-blur-md">
      <div className="flex items-center gap-4">
        ...
        {showBackButton ? (
          <button onClick={handleGoBack} className="flex items-center gap-2 text-white hover:text-orange-400 transition-colors">
            <ArrowLeft size={20} />
            <span className="hidden md:block">Back</span>
          </button>
        ) : (
          <h2 className="text-lg font-bold text-gradient-heading hidden md:block tracking-tight">{getPageTitle()}</h2>
        )}
      </div>
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[10px] opacity-80">
                          {isInternal && !user.isExpert ? 'Enroll as Expert' : 'Add personal details'}
                        </p>
                      </div>
                    </button>
                  )}
        ...
        {stats.map((stat, i) => (
          <div key={i} className="glass-card p-6 rounded-xl relative group">
            <div className="flex justify-between items-start mb-4">
              <div className="p-2.5 bg-white/5 rounded-lg text-white group-hover:scale-110 transition-transform">{stat.icon}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel p-5 rounded-xl">
            <h4 className="text-sm font-semibold text-gradient-heading mb-3 uppercase tracking-wider">System Status</h4>
...
const App: React.FC = () => {
  ...
  const [forceProfileModal, setForceProfileModal] = useState(false);
  ...
  useEffect(() => {
    const unsubscribe = mockAuthService.subscribeToAuth((currentUser) => {
      ...
      if (currentUser) {
        ...
        const isIncomplete = !currentUser.displayName || !currentUser.phoneNumber;
        if (onboard === 'profile' || isIncomplete) {
          setShowProfileModal(true);
          setForceProfileModal(isIncomplete);
        }
      }
    });
    return () => unsubscribe();
  ...
  const handleUpdateProfile = async (data: Partial<UserProfile>) => { // ✅ Added "data"
    if (user) {
      await mockAuthService.updateUserProfile(user.uid, data);
      setShowProfileModal(false);
      setForceProfileModal(false);
    }
  };
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/gst/compliance-guide" element={<GstComplianceGuide />} />
