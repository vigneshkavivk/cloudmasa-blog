  ...
  const GcpData = ({ selectedAccountId, accounts, onRefresh }) => {
    ...
    const [metrics, setMetrics] = useState({
      clusters: 0, vpcs: 0, loadBalancers: 0, buckets: 0, functions: 0,
      ...
      loadBalancerList: [], bucketsList: [], functionsList: [], vpcsList: [],
    });

    // ✅ ADD THESE MISSING STATES
    const [clusters, setClusters] = useState([]);
    const [instances, setInstances] = useState([]); // 👈 THIS WAS MISSING!
    const [activeTab, setActiveTab] = useState("clusters"); 

    const [lastSynced, setLastSynced] = useState(null);
    const [k8sData, setK8sData] = useState({ namespaces: [], workloads: [], services: [] });
