...
const ModuleConfigForm = ({ provider, moduleId, config, onConfigChange, vpcs = [] }) => {
  ...
  // After fetching AZs, auto-save them to config
useEffect(() => {
  if (moduleId === "vpc" && provider === "aws" && availableAZs.length > 0 && !config.availabilityZones) {
    updateConfig("availabilityZones", availableAZs.map(az => az.name));
  }
}, [availableAZs, moduleId, provider, config.availabilityZones]);

// ✅ Set default subnet counts for VPC module
useEffect(() => {
  if (moduleId !== "vpc" || provider !== "aws") return;

  let changed = false;
  const next = { ...config };

  if (next.publicSubnetCount  == null) { next.publicSubnetCount  = 2; changed = true; }
  if (next.privateSubnetCount == null) { next.privateSubnetCount = 2; changed = true; }
  if (next.subnetCount        == null) { next.subnetCount        = 4; changed = true; }
...
// Fetch AZs for EBS when region/account changes
useEffect(() => {
  if (moduleId === "ebs" && provider === "aws" && config.awsAccountId) {
    const fetchAZs = async () => {
      setLoadingAZs(true);
      try {
        const token = JSON.parse(localStorage.getItem('user'))?.token || '';
        ...
        if (data.success && Array.isArray(data.availabilityZones)) {
          setAvailableAZs(data.availabilityZones);
        } else throw new Error('Invalid AZ response');
      } catch (err) {
        console.error("Failed to fetch AZs for EBS:", err);
        setAvailableAZs([]);
      } finally {
        setLoadingAZs(false);
      }
    ...
    <div className="bg-[#1E2633] p-4 rounded-lg border border-[#3a5b9b] mb-4">
      ...
      <h3 className="text-lg font-medium mb-3 flex items-center gap-2">
        <span className="mr-1">
          ...
          {moduleId === "ebs" && <Globe className="text-teal-400" />}
  ...
  />
      ...
      {moduleId === "vpc" && provider === "aws" && (
        <>
          ...
          />
          {/* Public Subnet Count */}
{renderLabel("Public Subnet Count", "publicSubnetCount", "...", true)}
<select
  value={String(config.publicSubnetCount ?? 2)}   // ← safest
  onChange={(e) => {
    const pub = Number(e.target.value);
    const priv = Number(config.privateSubnetCount ?? 2);
    onConfigChange({
      ...config,
      publicSubnetCount: pub,
      subnetCount: pub + priv,
    });
  }}
  className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
>
...
{renderLabel("Private Subnet Count", "privateSubnetCount", "...", true)}
<select
  value={String(config.privateSubnetCount ?? 2)}   // ← important fallback
  onChange={(e) => {
    const priv = Number(e.target.value);
    const pub = Number(config.publicSubnetCount ?? 2);
    onConfigChange({
      ...config,
      privateSubnetCount: priv,
      subnetCount: pub + priv,
    });
  }}
  className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
            ...
            <div className="mt-4 p-4 bg-[#2A4C83] rounded-lg border border-[#3a5b9b]">
              ...
              {loadingAZs ? (
                ...
                <>
                  {/* Subnets Preview */}
<div className="mb-4">
  <h5 className="text-xs font-medium mb-2">Public Subnets ({config.publicSubnetCount || 0})</h5>
  {[...Array(config.publicSubnetCount || 0)].map((_, i) => {
    ...
    return (
      <div key={`pub-${i}`} className="p-2 bg-[#1E2633] rounded text-xs text-green-400">
        <span className="font-medium">Public Subnet {i + 1}</span> | AZ: {az} | CIDR: {subnetIp}
      </div>
    );
  })}

  <h5 className="text-xs font-medium mb-2 mt-3">Private Subnets ({config.privateSubnetCount || 0})</h5>
  {[...Array(config.privateSubnetCount || 0)].map((_, i) => {
    const offset = config.publicSubnetCount || 0;
    const parentCidr = config.cidrBlock || "10.0.0.0/16";
    const [baseIp, prefix] = parentCidr.split('/');
    const baseParts = baseIp.split('.').map(Number);
    const subnetIp = `${baseParts[0]}.${baseParts[1]}.${offset + i + 1}.0/${parseInt(prefix) + 8}`;
    const az = availableAZs[(offset + i) % availableAZs.length]?.name || `us-east-1${String.fromCharCode(97 + (offset + i) % 4)}`;
    return (
      <div key={`priv-${i}`} className="p-2 bg-[#1E2633] rounded text-xs text-blue-400">
        <span className="font-medium">Private Subnet {i + 1}</span> | AZ: {az} | CIDR: {subnetIp}
      </div>
    );
  })}
</div>
  ...
  "Statement": [{
    ...
    "Resource": "arn:aws:s3:::my-bucket/*"
      ...
      {moduleId === "ebs" && provider === "aws" && (
  <>
    {renderLabel("Volume Name", "name", "A unique name for your EBS volume (e.g., 'app-data-prod').", true)}
    <input
      type="text"
      value={config.name || ""}
      onChange={(e) => updateConfig("name", e.target.value)}
      ...
      className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
    />
    {/* Availability Zone - Auto-fetched */}
{renderLabel("Availability Zone", "availabilityZone", "Volume will be created in this AZ. Must match EC2 instance AZ if attaching later.", true)}
{loadingAZs ? (
  <p className="text-gray-400 text-sm mb-4">Loading AZs...</p>
) : (
  <select
    value={config.availabilityZone || ""}
    onChange={(e) => updateConfig("availabilityZone", e.target.value)}
    className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
    required
  >
    <option value="">-- Select AZ --</option>
    {availableAZs.map((az) => (
      <option key={az.name} value={az.name}>
        {az.name}
      </option>
    ))}
  </select>
)}
    {renderLabel("Volume Type", "volumeType", "gp3 = general purpose SSD (default); io1 = high-performance; st1 = throughput-optimized HDD.", true)}
    <select
      value={config.volumeType || "gp3"}
      onChange={(e) => updateConfig("volumeType", e.target.value)}
      className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
    >
      <option value="gp3">gp3 (General Purpose SSD)</option>
      <option value="gp2">gp2 (Legacy SSD)</option>
    ...
    {renderLabel("Size (GiB)", "size", "Volume size in gibibytes. gp3 min=1 GiB, max=16384 GiB.", true)}
    <input
      type="number"
      min="1"
      max="16384"
    ...
    {(config.volumeType === "io1" || config.volumeType === "io2") && (
      <>
        {renderLabel("IOPS", "iops", "Input/Output operations per second. For io1/io2 volumes.", true)}
        <input
          type="number"
          min="100"
          max="64000"
          value={config.iops || 3000}
          onChange={(e) => updateConfig("iops", parseInt(e.target.value) || 3000)}
          className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
        />
      </>
    )}
    ...
    {config.volumeType === "gp3" && (
      <>
        {renderLabel("Throughput (MiB/s)", "throughput", "For gp3 volumes. Range: 125–1000 MiB/s.", false)}
        <input
          type="number"
          min="125"
          max="1000"
          value={config.throughput || 125}
          onChange={(e) => updateConfig("throughput", parseInt(e.target.value) || 125)}
          className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
        />
      </>
    )}
    ...
    <div className="flex items-center mb-4">
      <input
        ...
        className="rounded text-orange-500 mr-2"
      />
      {renderLabel("Encrypt Volume", "encrypted", "Encrypts data at rest using AWS KMS. Recommended for production.", false)}
    </div>

    {renderLabel("Environment Tag", "environment", "Used for cost tracking (e.g., dev, staging, prod).")}
    <select
      value={config.environment || "prod"}
      onChange={(e) => updateConfig("environment", e.target.value)}
      className="w-full bg-[#2A4C83] border border-[#3a5b9b] rounded-md p-2 text-white mb-4"
