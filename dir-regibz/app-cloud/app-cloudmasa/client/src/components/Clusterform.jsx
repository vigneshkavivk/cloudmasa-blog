...
import { useAuth } from '../hooks/useAuth'; // ✅ RBAC hook for permission control

function ClusterForm() {
  const { hasPermission } = useAuth();
  const canAccessTerminal = 
    hasPermission('Agent', 'Configure') && 
    hasPermission('Credentials', 'View');

  if (!canAccessTerminal) {
    return (
      <div className="min-h-screen flex items-center justify-center py-10 px-4" style={{ backgroundColor: '#1E2633' }}>
        <div className="max-w-md w-full p-6 rounded-xl shadow-md border border-gray-700" style={{ backgroundColor: '#2A4C83' }}>
          <div className="text-center">
            <Lock className="h-12 w-12 mx-auto text-red-500 mb-4" />
            <h2 className="text-xl font-bold text-red-400 mb-2">🔒 Access Denied</h2>
            <p className="text-gray-300">
              You need <span className="font-mono">Agent.Configure</span> and <span className="font-mono">Credentials.View</span> permissions to interact with AWS.
            </p>
          </div>
        </div>
      </div>
  ...
  const fetchClusters = async () => {
    ...
    setError(null);
    try {
      // ✅ Correct endpoint + valid payload
