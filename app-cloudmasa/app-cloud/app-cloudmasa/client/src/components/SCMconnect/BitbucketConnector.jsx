// src/components/BitbucketConnector.jsx
...
const BitbucketConnector = ({ onBack }) => {
  ...
  const handleOwnConnect = async (e) => {
    ...
    if (!finalToken) return toast.error(`Bitbucket App Password is required!`);

    try {
      const payload = { bitbucketToken: finalToken };
      const res = await api.post("/api/scm/fetch-bitbucket-repos", payload);

      const repoNames = res.data.values.map((repo) => ({
        name: repo.name,
      ...
      setRepos(repoNames);
      toast.success(`✅ Bitbucket repositories fetched!`);
      await api.patch("/api/users/me", { bitbucketToken: finalToken });
      toast.success(`🔒 Bitbucket token saved for future sessions.`);
    } catch (err) {
      console.error("Connection error:", err);
      toast.error(`❌ Invalid Bitbucket token or API error.`);
    }
  };
  ...
  const fetchFolders = async (repoFullName) => {
    ...
    try {
      const finalToken = tokenInput.trim();
      if (!finalToken) {
        toast.error(`Please reconnect with your Bitbucket token.`);
        return;
      }

      const payload = {
        bitbucketToken: finalToken,
        owner: repo.owner,
        repo: repo.name,
      };
      const res = await api.post("/api/scm/fetch-bitbucket-folders", payload);
  ...
  const saveRepoAndAllFolders = async () => {
    ...
    try {
      ...
      const allFolders = res.data.values.map((item) => ({
        name: item.path.split("/").pop(),
        path: item.path,
      }));
      ...
      const accountType = "Client Account"; // Bitbucket doesn't have a default mode
      ...
      const skipFolders = [".bitbucket", "docs", "scripts", "tests", "test", "__pycache__"];
      for (const folder of allFolders) {
        ...
        const folderUrl = `https://bitbucket.org/${selectedRepo}/src/main/${encodeURIComponent(folder.path)}`;
        const folderPayload = {
          userId: username,
  ...
  const saveFolderOnly = async () => {
    ...
    try {
      ...
      const folderUrl = `https://bitbucket.org/${selectedRepo}/src/main/${encodeURIComponent(selectedFolder)}`;
    ...
    <div className="min-h-screen px-6 py-10 text-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-6 mt-6 mb-6">
          {/* Bitbucket does not have a default mode, so we only show "Client Account" */}
          <button
            onClick={() => setMode("own")}
          ...
          <div className="mt-4 bg-[#1e293b]/80 backdrop-blur border border-blue-900/50 rounded-lg shadow-lg p-6">
            {mode === "own" && (
              <form onSubmit={handleOwnConnect} className="space-y-4 mt-6">
                <div>
                  <label className="block text-gray-200 mb-1">Bitbucket App Password</label>
                  <input
                    type="password"
                    value={tokenInput}
              ...
              <tbody>
                {selectedRepo && selectedFolder ? (
                  <tr className="border-b border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td className="p-3">
                      <a
                        href={`https://bitbucket.org/${selectedRepo}/src/main/${encodeURIComponent(selectedFolder)}`}
                        target="_blank"
