// src/components/AccountComponent.jsx
...
const AccountComponent = ({ awsConfig }) => {
  ...
  // ✅ Require BOTH permissions to use terminal
  const canAccessTerminal =
    hasPermission('Agent', 'Configure') &&
    hasPermission('Credentials', 'View');

  // 🔒 Block access if user lacks permissions
  if (!canAccessTerminal) {
    return (
      <div className="bg-gray-950 rounded-xl border border-gray-800 shadow-2xl max-w-3xl mx-auto p-6 text-center">
        <div className="flex justify-center mb-4">
          <Lock className="w-12 h-12 text-red-500" />
        </div>
        <h3 className="text-xl font-bold text-red-400 mb-2">Access Denied</h3>
        <p className="text-gray-400">
          You need <span className="font-mono">Agent.Configure</span> and{" "}
          <span className="font-mono">Credentials.View</span> permissions to access the terminal.
        </p>
      </div>
    );
  }
