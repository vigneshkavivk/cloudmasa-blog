...
import { TerminalSquare, X, Lock } from "lucide-react";
...
const TerminalComponent = ({ account, onClose }) => {
  ...
  const canAccessTerminal = hasPermission('Agent', 'Configure');

  // 🔒 Block access if user lacks permission
  if (!canAccessTerminal) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70 backdrop-blur-sm">
        <div className="w-full max-w-md bg-[#1a1a1a] rounded-2xl p-6 border border-gray-800 text-center">
          ...
          <p className="text-gray-300 mb-4">
            You need <span className="font-mono">Agent.Configure</span> permission to use this terminal.
          </p>
          <button
            onClick={onClose}
  ...
  // Set up AWS credentials if account is provided
  useEffect(() => {
    if (account) {
  ...
  // Pre-fill AWS configure commands
  const setupAwsConfigure = (acc) => {
    ...
    // Execute with slight delay for visual clarity
    cmds.forEach((cmd, i) => {
      setTimeout(() => executeCommand(cmd), i * 800);
    });
  };
        ...
        <div className="flex flex-wrap gap-2 mb-4 px-1">
          {[
            ...
            <button
              ...
              className="bg-gray-800 text-white px-3 py-1 rounded hover:bg-gray-700 text-sm whitespace-nowrap"
            >
              {btn.label}
            </button>
          ))}
