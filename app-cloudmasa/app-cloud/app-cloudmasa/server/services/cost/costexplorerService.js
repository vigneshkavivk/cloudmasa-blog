//services/cost/costexplorerService.js
...
export const getCostByTag = async (accountId, tagKey, tagValue) => {
  const { account, cleanId, useDirectAccess } = await findAccountByAccountId(accountId);
  const credentials = await getCredentialsForAccount(accountId, cleanId, useDirectAccess, account.roleArn);
  ...
  const params = {
    TimePeriod: { Start: getUTCDateStr(start), End: getUTCDateStr(end) },
    Granularity: 'DAILY',
    Metrics: ['UnblendedCost'],
    Filter: {
  ...
  return await ce.getCostAndUsage(params).promise();
};
