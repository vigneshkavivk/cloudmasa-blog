  ...
  return (
    <div className="bg-gradient-to-r from-emerald-900/30 to-emerald-800/10 border border-emerald-500/20 rounded-xl p-4 md:p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center shadow-lg mb-8 relative overflow-hidden backdrop-blur-sm">
      ...
      <div className="text-left sm:text-right z-10">
        ...
        <p className="text-slate-200 font-mono font-medium text-sm md:text-base">FSSAI-2026-7K3M9</p>
      </div>
    </div>
  );
};
      ...
      >
        ...
        <ul className="space-y-2.5">
          {[
            ...
            { label: 'Proof of Premises', key: 'premiseProof' },
            { label: 'Photo ID (Aadhaar)', key: 'photoId' },
            { label: 'Layout Plan (if manufacturing)', key: 'layoutPlan' },
          ].map((item, idx) => {
...
export default function FssaiLicenseForm() {
  ...
  const getStepFields = (step: number): (keyof FormData)[] => {
    switch (step) {
      case 1:
        return ['businessName', 'pan', 'constitution', 'turnover', 'premiseType'];
      case 2:
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const isLayoutRequired = formData.premiseType === 'manufacturing';
    if (!uploadedFiles.panCard || !uploadedFiles.premiseProof || !uploadedFiles.photoId || (isLayoutRequired && !uploadedFiles.layoutPlan)) {
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[
                ...
                { key: 'premiseProof', label: 'Proof of Premises' },
                { key: 'photoId', label: 'Photo ID (Aadhaar)' },
                ...(formData.premiseType === 'manufacturing' ? [{ key: 'layoutPlan', label: 'Layout Plan' }] : []),
              ].map((doc) => (
          ...
          <div className="pt-4 border-t border-slate-800">
            <p className="text-xs text-slate-500">
              This is a preview only. No data is submitted until you click <span className="text-emerald-400 font-medium">Submit Application</span>.
            </p>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-900/80 backdrop-blur-xl p-8 rounded-2xl shadow-2xl max-w-md w-full text-center border border-slate-700/50">
          ...
          <p className="text-slate-300 mb-8">
            ...
            <span className="text-sky-400 font-mono">FSSAI-2026-7K3M9</span>.
          </p>
                    ...
                    <div>
                      <fieldset className="space-y-4 mb-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          />
                          <FileUploader
                            label="Photo ID (Aadhaar)"
                            name="photoId"
                            required
                            onChange={handleFileUpload('photoId')}
                          />
                          <FileUploader
                            label={formData.premiseType === 'manufacturing' ? "Layout Plan (Mandatory)" : "Layout Plan (Optional)"}
                            name="layoutPlan"
