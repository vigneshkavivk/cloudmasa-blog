...
const Documents: React.FC<DocumentsProps> = ({ user }) => {
      ...
      if (isAdminView) {
          ...
          const report = LEGAL_TEMPLATES.map(template => {
              ...
              return {
                  ...
                  recipients: matches.map(m => m.user.displayName).slice(0, 5) // Last 5 recipients
              };
          });
          setAdminReportData(report);
      } else {
          // Fetch Personal Vault
          const [fetchedDocs, fetchedFolders] = await Promise.all([
            mockDbService.getDocuments(user.uid),
            mockDbService.getFolders(user.uid)
          ]);
          setDocs(fetchedDocs);
  ...
  const generateLegalContent = (template: LegalTemplate, data: typeof legalForm) => {
    ...
    // Simulating professional content generation
    switch(template.id) {
      case 'nda':
        body = `
    ...
    const custom = data.clause ? `<div class="mt-6 p-4 bg-gray-800/50 rounded border border-gray-700"><h3 class="font-bold text-sm mb-2 text-orange-400">Additional Clauses</h3><p class="text-sm italic text-gray-300">${data.clause}</p></div>` : '';
  ...
  const handleCreateFolder = async (e: React.FormEvent) => {
    ...
    try {
      const newFolder = await mockDbService.createFolder(newFolderName, user.uid);
  ...
  const handleGenerateLegalDoc = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newDoc: ServiceDocument = {
        ...
        formData: { ...legalForm },
        userId: user.uid,
        folderId: 'personal' // FORCE SAVE TO PERSONAL FOLDER
      };
      ...
      // Auto-switch to personal folder to show the new doc
      setSelectedFolderId('personal');
  ...
  const getDocIcon = (type: string) => {
    switch(type) {
      ...
      case 'legal': return <FileSignature size={20} />;
  ...
  // --- NORMAL VAULT VIEW ---
  return (
    <div className="flex h-[calc(100vh-64px)] overflow-hidden animate-fade-in relative" onClick={() => { setOpenDropdownId(null); setOpenFolderMenuId(null); }}>
      
      {/* --- Left Rail (Folders) --- */}
      <aside className={`absolute md:relative z-20 w-64 h-full glass-panel border-r border-white/5 flex flex-col transition-transform duration-300 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 bg-navy-900 md:bg-transparent`}>
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          <h3 className="font-bold text-gray-400 text-xs uppercase tracking-wider">My Vault</h3>
          <button onClick={() => setShowCreateFolder(true)} className="p-1.5 hover:bg-white/5 rounded-lg text-orange-500 transition-colors" title="New Folder">
            <Plus size={16} />
          </button>
        </div>
            ...
            <div key={folder.id} className="relative group">
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  <Folder size={16} className={`flex-shrink-0 ${selectedFolderId === folder.id ? 'fill-orange-500/20' : ''}`} />
                ...
                <div className="flex items-center">
                  ...
                  <div 
                    ...
                    className={`p-1 rounded hover:bg-white/10 text-gray-500 hover:text-white transition-colors cursor-pointer ${folder.type === 'system' ? 'invisible' : 'visible'}`}
                  >
              ...
              {openFolderMenuId === folder.id && (
                <div className="absolute right-0 top-full z-30 mt-1 w-32 bg-navy-900 border border-white/10 rounded-lg shadow-xl py-1 animate-fade-in">
                   <button 
                     onClick={(e) => { e.stopPropagation(); setFolderToEdit(folder); setNewFolderName(folder.name); setShowRenameFolder(true); setOpenFolderMenuId(null); }}
                     className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/5 hover:text-white flex items-center gap-2"
                   >
                     <Edit2 size={12} /> Rename
                   </button>
                   <button 
                     onClick={(e) => { e.stopPropagation(); setFolderToEdit(folder); setShowDeleteFolder(true); setOpenFolderMenuId(null); }}
        ...
        <button onClick={() => setMobileMenuOpen(false)} className="md:hidden absolute top-2 right-2 p-2 text-gray-500">✕</button>
      </aside>
      
      {/* Mobile Backdrop */}
      {mobileMenuOpen && <div className="fixed inset-0 bg-black/50 z-10 md:hidden" onClick={() => setMobileMenuOpen(false)}></div>}

      {/* --- Main Content --- */}
      <main className="flex-1 flex flex-col min-w-0 bg-[#020c1b]">
        {/* Toolbar */}
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {filteredDocs.map((doc) => (
                <div key={doc.id} className="glass-card rounded-xl p-4 group relative hover:border-orange-500/30 transition-all">
                  ...
                  {openDropdownId === doc.id && (
                    <>
                      ...
                      <div className="absolute top-10 right-2 z-20 w-40 bg-navy-900 border border-white/10 rounded-lg shadow-xl py-1 animate-fade-in">
                        <button 
                          onClick={() => { setActionDoc(doc); setActionType('move'); setShowMoveCopy(true); setOpenDropdownId(null); }}
                        ...
                        <button 
                          onClick={() => { setActionDoc(doc); setActionType('copy'); setShowMoveCopy(true); setOpenDropdownId(null); }}
                          className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-white/5 hover:text-white flex items-center gap-2"
                        >
                  ...
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setOpenDropdownId(openDropdownId === doc.id ? null : doc.id); }} className="p-1 text-gray-500 hover:text-white rounded hover:bg-white/10">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    {doc.serviceId ? (
                       <span className="text-[10px] font-mono text-gray-500 bg-white/5 px-1.5 py-0.5 rounded">{doc.serviceId}</span>
                    ) : (
                       <span className="text-[10px] font-mono text-gray-500 bg-white/5 px-1.5 py-0.5 rounded capitalize">{doc.subtype || doc.type}</span>
                    )}
                    {doc.status && (
                       <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase ${
                           doc.status === 'submitted' ? 'bg-yellow-500/10 text-yellow-500' : 
                           doc.status === 'processing' ? 'bg-blue-500/10 text-blue-400' : 
                           doc.status === 'approved' ? 'bg-orange-500/10 text-orange-400' : 
          ...
          <div className="glass-panel rounded-2xl w-full max-w-sm p-6 border border-white/10">
            ...
            <form onSubmit={handleCreateFolder}>
              ...
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setShowCreateFolder(false)} className="px-4 py-2 text-sm text-gray-400 hover:text-white">Cancel</button>
                <button 
                  type="submit" 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className="glass-panel rounded-2xl w-full max-w-sm p-6 border border-white/10">
            ...
            <form onSubmit={handleRenameFolder}>
              ...
              />
              <div className="flex justify-end gap-3">
                <button type="button" onClick={() => setShowRenameFolder(false)} className="px-4 py-2 text-sm text-gray-400 hover:text-white">Cancel</button>
                <button 
                  type="submit" 
                  disabled={!newFolderName.trim() || isSubmitting}
                  className="px-4 py-2 bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-400 hover:to-red-500 text-white rounded-lg text-sm font-bold disabled:opacity-50"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className="glass-panel rounded-2xl w-full max-w-sm p-6 border border-white/10">
            ...
            <p className="text-sm text-gray-400 mb-6 leading-relaxed">
              This will <strong>move all documents</strong> inside this folder to your <strong>Personal</strong> folder. The folder itself will be deleted.
            </p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowDeleteFolder(false)} className="px-4 py-2 text-sm text-gray-400 hover:text-white">Cancel</button>
              <button 
      ...
      {/* Generate Legal Doc Modal (2-Step) */}
      {showLegalModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className={`glass-panel rounded-2xl w-full border border-white/10 transition-all ${legalStep === 'catalog' ? 'max-w-4xl h-[80vh] flex flex-col' : 'max-w-lg'}`}>
            ...
            <div className="p-6 border-b border-white/10 flex justify-between items-center bg-navy-950/50">
               <div>
                  ...
                  <p className="text-sm text-gray-400">
                     {legalStep === 'catalog' ? 'Select a professionally drafted template to get started.' : 'Fill in the details to generate your document.'}
                  </p>
               </div>
               <button onClick={() => setShowLegalModal(false)} className="text-gray-400 hover:text-white p-1 hover:bg-white/10 rounded">✕</button>
            </div>
               ...
               <div className="flex-1 overflow-y-auto p-6 bg-navy-900/50">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                     {LEGAL_TEMPLATES.map(template => (
                        <div key={template.id} className="glass-card p-5 rounded-xl border border-white/5 hover:border-orange-500/30 group transition-all flex flex-col">
                           <div className="flex justify-between items-start mb-4">
                              ...
                              <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded border ${
                                 template.category === 'Agreement' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                                 template.category === 'Notice' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' :
                                 template.category === 'HR' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' :
            ...
            {legalStep === 'form' && selectedTemplate && (
              <div className="p-6">
                 <button onClick={() => setLegalStep('catalog')} className="flex items-center text-xs text-gray-400 hover:text-white mb-4 transition-colors">
                 ...
                 <form onSubmit={handleGenerateLegalDoc} className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3 mt-6">
                     <button type="button" onClick={() => setShowLegalModal(false)} className="px-4 py-2 text-sm text-gray-400 hover:text-white">Cancel</button>
                     <button 
                       type="submit" 
                       disabled={isSubmitting}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={(e) => e.stopPropagation()}>
          <div className="glass-panel rounded-2xl w-full max-w-sm p-0 border border-white/10 overflow-hidden">
            ...
            <div className="max-h-60 overflow-y-auto p-2">
              {folders.filter(f => actionType === 'copy' || f.id !== actionDoc.folderId).map(f => (
                <button 
                  key={f.id}
            ...
            <div className="p-3 bg-black/20 border-t border-white/5 text-center">
              <button onClick={() => setShowMoveCopy(false)} className="text-xs text-gray-500 hover:text-gray-300">Cancel</button>
            </div>
          </div>
        </div>
      )}
             ...
             <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
                <div>
                   ...
                   <div className="flex gap-2 mt-1">
                      ...
                      {viewDoc.type === 'legal' && <span className="text-xs font-mono text-blue-400 bg-blue-500/10 px-1 rounded uppercase">{viewDoc.subtype}</span>}
                      <span className="text-xs text-gray-500">in {folders.find(f => f.id === viewDoc.folderId)?.name}</span>
                   </div>
                </div>
                ...
                {viewDoc.type === 'legal' && viewDoc.formData && (
                   <div className="mt-8 pt-6 border-t border-white/5 grid grid-cols-2 gap-4">
                      <div className="p-3 rounded-lg border border-white/5 bg-white/[0.02]">
                         <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Recipient</label>
                         <p className="text-sm text-gray-200">{viewDoc.formData.recipient}</p>
                      </div>
                      <div className="p-3 rounded-lg border border-white/5 bg-white/[0.02]">
                         <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Effective Date</label>
                         <p className="text-sm text-gray-200">{viewDoc.formData.date}</p>
                      </div>
                   </div>
                )}

                {(viewDoc.type !== 'note' && viewDoc.type !== 'file' && viewDoc.type !== 'legal' && viewDoc.formData) && (
                ...
                {viewDoc.type === 'file' && (
                  <div className="flex items-center justify-center py-10 border-2 border-dashed border-white/10 rounded-xl bg-black/20">
                     <div className="text-center">
                        ...
                        <button className="mt-4 px-4 py-2 bg-white/5 hover:bg-white/10 rounded-lg text-sm text-orange-400">Download File</button>
                     </div>
                  </div>
                )}
             </div>
